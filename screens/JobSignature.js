...
const JobSignature = ({ navigation, route }) => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data.technicianSignature) {
                        setSavedSignature(data.technicianSignature)
                    }
                } else {
                    console.log('No signature document found for this job.')
                }
            } catch (error) {
    ...
    const handleSignature = async (signature) => {
        ...
        try {
            ...
            const fileUri =
                FileSystem.documentDirectory + `${jobNo}-${workerId}.png`
            ...
            // Update Firestore document with the download URL at the correct path
            const docRef = doc(
                db,
                'workerAttendance',
                workerId,
            ...
            const docSnap = await getDoc(docRef)
            if (!docSnap.exists()) {
                // If the document does not exist, create it instead of updating
                await setDoc(docRef, {
                    technicianSignature: downloadURL,
                    // Add any other fields needed
                })
                console.log('Signature saved successfully as a new document.')
            } else {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Header title="Technician Signature" />
