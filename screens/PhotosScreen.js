...
const PhotosScreen = ({ navigation, route }) => {
    ...
    const uploadPhotoToStorage = async (uri, jobNo, workerId) => {
        const storage = getStorage()
        const response = await fetch(uri)
        const blob = await response.blob()

        const photoRef = ref(storage, `jobPhotos/${jobNo}-${workerId}-JobPhoto`)
    ...
    const savePhoto = async (uri) => {
        try {
            const jobNo = route.params?.jobNo
            const workerId = route.params?.workerId // Replace with actual workerId if available

            const downloadURL = await uploadPhotoToStorage(uri, jobNo, workerId)
