...
const TasksScreen = ({ navigation, route }) => {
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      const maxId = Math.max(...jobData.taskList.map(task => 
            ...
            <View style={styles.badgeContainer}>
              {task.isRequired && (
                <View style={styles.requiredBadge}>
                  <Icon name="star" size={16} color="#DC2626" />
                  <Text style={styles.requiredText}>Priority</Text>
                </View>
              )}
            </View>
          </View>
      ...
      <View style={styles.taskFooter}>
        <View style={styles.taskMetadata}>
          <Text style={styles.timestampText}>
            {task.createdAt ? `Created: ${format(new Date(task.createdAt), 'MMM d, yyyy h:mm a')}` : ''}
          </Text>
          {task.completed && (
            <>
              {task.completedBy && (
                <Text style={styles.completedByText}>
                  Completed by: {workerDetails[task.completedBy]?.fullName || 'Unknown Worker'}
                </Text>
              )}
              {task.completionDate && (
                <Text style={styles.timestampText}>
                  on {format(new Date(task.completionDate), 'MMM d, yyyy h:mm a')}
                </Text>
              )}
            </>
          )}
            ...
            <View style={styles.statsGrid}>
              <View style={styles.statItem}>
                <View style={[styles.iconCircle, { backgroundColor: '#EBF5FF' }]}>
                  <Icon name="assignment" size={24} color="#4a90e2" />
                </View>
                <Text style={styles.statCount}>{taskStats.total}</Text>
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statCount}>{taskStats.completed}</Text>
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statCount}>{taskStats.pending}</Text>
                <Text style={styles.statTitle}>Pending</Text>
              </View>

              <View style={styles.statItem}>
                <View style={[styles.iconCircle, { backgroundColor: '#FEE2E2' }]}>
                  <Icon name="priority-high" size={24} color="#EF4444" />
                </View>
                <Text style={styles.statCount}>{taskStats.priority}</Text>
          ...
          <View style={styles.tasksContainer}>
            ...
            {tasks.map(task => (
              <TaskItem
                key={task.id}
                task={task}
                onPress={() => handleUpdateStatus(task.id, !task.completed)}
              />
            ))}
          </View>
        </View>
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.priorityToggle}>
              <TouchableOpacity
                onPress={() => setNewTask(prev => ({ ...prev, isRequired: !prev.isRequired }))}
                style={styles.checkboxContainer}
              >
                <View style={[
                  styles.checkbox,
                  newTask.isRequired && styles.checkboxChecked
                ]}>
                  {newTask.isRequired && <Icon name="check" size={16} color="#fff" />}
                </View>
                <Text style={styles.checkboxLabel}>Priority</Text>
              </TouchableOpacity>

              <TouchableOpacity
                onPress={() => setNewTask(prev => ({ ...prev, requiredNotes: !prev.requiredNotes }))}
                style={styles.checkboxContainer}
              >
                <View style={[
                  styles.checkbox,
                  newTask.requiredNotes && styles.checkboxChecked
                ]}>
                  {newTask.requiredNotes && <Icon name="check" size={16} color="#fff" />}
                </View>
                <Text style={styles.checkboxLabel}>Requires Notes</Text>
              </TouchableOpacity>

              <TouchableOpacity
                onPress={() => setNewTask(prev => ({ ...prev, requiredPhotos: !prev.requiredPhotos }))}
                style={styles.checkboxContainer}
              >
                <View style={[
                  styles.checkbox,
                  newTask.requiredPhotos && styles.checkboxChecked
                ]}>
                  {newTask.requiredPhotos && <Icon name="check" size={16} color="#fff" />}
                </View>
                <Text style={styles.checkboxLabel}>Requires Photos</Text>
              </TouchableOpacity>
            </View>
