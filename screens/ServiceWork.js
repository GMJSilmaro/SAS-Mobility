...
const ServiceWork = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchPhotos = async () => {
            try {
                ...
                if (!jobNo || !workerId) {
                    console.log(
                        'No jobNo or workerId provided, skipping photo fetch'
                    )
                    return
                }
                ...
                if (jobSnap.exists()) {
                    const jobData = jobSnap.data()
                    const photoList = jobData.photos || []
                    console.log('Photos from Firebase:', photoList)
                    console.log('Total photos:', photoList.length)
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        ...
                        // Call the function to add worker status when starting work
                        const workerStatusAdded = await addWorkerStatus()

                        // Start the timer only after the worker status is confirmed as added
                        if (workerStatusAdded) {
                            console.log(
                                'Worker status successfully added. Starting timer...'
                            )
    ...
    const addWorkerStatus = async () => {
        console.log('Entering addWorkerStatus...')

        const jobNo = route?.params?.jobNo
        const workerId = route?.params?.workerId // Assuming this comes from route params
        const workerStatusID = `WS-${workerId}`

        try {
            // Retrieve worker data from the users collection
            const workerDocRef = doc(db, 'users', workerId) // Adjust the path if necessary
            ...
            const workerName = workerData.fullName || 'Unknown Worker' // Get workerName or default
            ...
            const workerStatusRef = doc(
                ...
                jobNo,
                'workerStatus',
                workerStatusID // Make sure to generate this uniquely as needed
            )

            // Setting fields directly without a nested object
            await setDoc(workerStatusRef, {
                workerStatusID, // Ensure this is unique
                workerID: workerId,
                workerName: workerName,
                jobNo: jobNo,
                status: 'Job Started', // Example status
                progress: '', // Default progress; update as needed
                jobClockIn: new Date().toISOString(), // Current timestamp for clock-in
                jobClockOut: null, // Set to null initially; update later
                startDate: new Date().toISOString(), // Current date as start date
                endDate: null, // Set to null initially; update later
                completionTime: null, // Set to null initially; update later
                breakDuration: '1hr', // Default break duration; update as needed
                jobDuration: '', // Default job duration; update as needed
                technicianSignature: {
                    signatureURL: '', // Provide the URL of the signature when available
                    signatureTimestamp: null, // Set to null initially; update later
                },
                notes: '', // Optional notes field
            })

            console.log('Worker status added successfully.')
            return true
        } catch (error) {
            console.error('Error adding worker status:', error)
            return false
        }
    }
        ...
        if (!isOnBreak) {
            ...
            const interval = setInterval(() => {
                ...
                const actualElapsedTime =
                    totalElapsedTime - (totalPausedTime + pauseDuration)
    ...
    const handleEndWork = async () => {
        ...
        // Calculate the total time of the job from start to end
        const totalJobDuration = new Date() - timeTracking.startTime // Total job duration in milliseconds

        // Calculate total working time by subtracting total paused time from elapsed time
        const totalWorkingTime =
            totalJobDuration -
            (totalPausedTime + (isOnBreak ? new Date() - lastPauseTime : 0)) // in milliseconds

        // Calculate break time duration
        const breakTimeDuration = totalJobDuration - totalWorkingTime // in milliseconds

        try {
            // Reference to the worker status document in Firestore
            const workerStatusID = `WS-${route?.params?.workerId}` // Format workerStatusID
            const workerStatusRef = doc(
                db,
                'jobs',
                route?.params?.jobNo,
                'workerStatus',
                workerStatusID // Use the generated ID
            )
            ...
            // Update the time tracking state to include the end time and break duration
            setTimeTracking((prev) => ({
                ...prev,
                endTime: new Date(),
                breakTimeDuration: breakTimeDuration, // Save break time duration in the time tracking state
            }))
        ...
        <SafeAreaView style={styles.container}>
            ...
            <TouchableOpacity
                style={[
                    ...
                    (!allTasksCompleted || !timeTracking.endTime) &&
